#!/usr/bin/env python3
"""MediaPipe + RealSense 손바닥 3D 자동 테스트 노드.

실행 즉시 별도의 Action Goal 없이 손바닥 검출과 base 좌표 계산을 시작한다.
기존 /find_hand_order Action Server 기능도 유지한다.
...
class MediaPipePalm3DActionServer(Node):
    ...
    def __init__(self) -> None:
        super().__init__("mediapipe_palm_3d_auto_test")
        ...
        self.last_test_log_time = 0.0
        self.last_test_wait_log_time = 0.0
        ...
        # 테스트 모드에서는 Action Goal을 기다리지 않고 즉시 검출을 시작한다.
        self.find_hand_active = bool(self.auto_test_mode)
        if self.find_hand_active:
        ...
        self.get_logger().warning(
            "AUTO TEST MODE enabled: hand detection starts immediately "
            "without an Action Goal"
        )
        self.get_logger().warning(
            f"Base Z correction enabled: z=max(0, z-{self.base_z_correction_mm:.1f} mm)"
        )
        if not self.show_window:
            self.get_logger().info(
                "Headless mode enabled: OpenCV window is disabled for ROS 2 thread safety"
            )
    ...
    def _declare_parameters(self, script_dir: Path) -> None:
        defaults = {
            ...
            "auto_test_mode": True,
            "test_log_interval_sec": 0.5,
            "base_z_correction_mm": 250.0,
            "show_window": False,
            "mirror_view": False,
    ...
    def _read_parameters(self) -> None:
        names = [
            ...
            "auto_test_mode",
            "test_log_interval_sec",
            "base_z_correction_mm",
            "show_window",
            "mirror_view",
            "window_name",
        ]
    ...
    def _validate_parameters(self) -> None:
        ...
        if float(self.test_log_interval_sec) <= 0:
            raise ValueError("test_log_interval_sec must be > 0")
        if float(self.base_z_correction_mm) < 0:
            raise ValueError("base_z_correction_mm must be >= 0")
    ...
    async def execute_find_hand(self, goal_handle) -> FindOrder.Result:
        ...
        try:
            ...
            self.find_hand_active = bool(self.auto_test_mode)
            self.current_feedback_state = "searching" if self.find_hand_active else "idle"
            with self._goal_lock:
                self._goal_reserved = False
    ...
    def _camera_to_base_mm(
        ...
        # link_6 기준 보정: 250 mm를 빼고 음수는 0으로 제한한다.
        base_point_mm = np.asarray(base_point_mm, dtype=np.float64).copy()
        base_point_mm[2] = max(
            0.0,
            float(base_point_mm[2]) - float(self.base_z_correction_mm),
        )
    ...
    def _synced_callback(self, color_msg: Image, depth_msg: Image) -> None:
        ...
        if not result.multi_hand_landmarks:
            if (
                self.auto_test_mode
                and now - self.last_test_wait_log_time >= 2.0
            ):
                self.get_logger().info("[자동 테스트] 손을 찾는 중...")
                self.last_test_wait_log_time = now
            self.stable_frame_count = 0
            self.last_stable_point_mm = None
            self.current_feedback_state = "searching"
            self._publish_detected(False)
        ...
        if depth_value_mm is None:
            if (
                ...
                self.get_logger().warning(
                    "[자동 테스트] 손은 검출됐지만 유효한 Depth가 없습니다"
                )
                self.last_test_wait_log_time = now
            self.stable_frame_count = 0
            self.last_stable_point_mm = None
        ...
        if base_raw_mm is None:
            if (
                ...
                self.get_logger().warning(
                    f"[자동 테스트] TF 대기 중: {self.base_frame} <- "
                    f"{self.calibration_frame}"
                )
                self.last_test_wait_log_time = now
            self.stable_frame_count = 0
            self.last_stable_point_mm = None
        ...
        if (
            self.auto_test_mode
            and now - self.last_test_log_time
            >= float(self.test_log_interval_sec)
        ):
            cx, cy, cz = self.filtered_camera_xyz_mm
            bx, by, bz = self.filtered_base_xyz_mm
            self.get_logger().info(
                "[자동 테스트] "
                f"camera=({cx:.1f}, {cy:.1f}, {cz:.1f}) mm, "
                f"base=({bx:.1f}, {by:.1f}, {bz:.1f}) mm, "
                f"stable={self.stable_frame_count}/{self.stable_frames}"
            )
            self.last_test_log_time = now
    ...
    def _show_waiting(self, image: np.ndarray, text: str = "AUTO TEST: SEARCHING HAND") -> None:
        cv2.putText(
            image,
            text,
            (15, 30),
    ...
    def _show(self, image: np.ndarray) -> None:
        # OpenCV HighGUI/Qt is not thread-safe when called from a ROS 2
        # MultiThreadedExecutor callback. Keep the Action Server headless by
        # default. If GUI is explicitly enabled and OpenCV raises an error,
        # disable it instead of terminating the ROS node.
        if not self.show_window:
            return
        ...
        try:
            ...
            self.get_logger().error(
                f"OpenCV window disabled after GUI error: {error}"
            )
