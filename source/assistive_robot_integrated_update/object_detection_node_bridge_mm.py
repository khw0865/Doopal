...
객체의 base 좌표 x/y/z와 Action/Service coordinate는 mm 단위로 통일함.
카메라 raw depth는 기존 Grip 파트 호환을 위해 m 단위를 유지함.
...
class ObjectDetectionNode(Node):
    def __init__(
        ...
        hand_eye_transform_path=str(Path(__file__).resolve().parent / 'T_gripper2camera.npy'),
    ...
    def run_inference_on_latest_frame(self, conf_threshold, target_label=None):
        ...
        for r in results:
            for box in r.boxes:
                ...
                bx_mm = float(bx) * 1000.0
                by_mm = float(by) * 1000.0
                bz_mm = float(bz) * 1000.0

                detections.append({
                    'class_name': label,
                    'confidence': confidence,
                    'coordinate_unit': 'mm',
                    'x': round(bx_mm, 2),              # base_link 기준 mm
                    'y': round(by_mm, 2),
                    'z': round(bz_mm, 2),
                    'camera_depth_z': round(float(coord_result.depth_m), 4),  # 기존 Grip 파트 호환: m
                    'bbox_width': x2 - x1,
                    'bbox_height': y2 - y1,
                })
...
def main():
    model_path = str(Path(__file__).resolve().parent / 'my_seg_best.pt')
