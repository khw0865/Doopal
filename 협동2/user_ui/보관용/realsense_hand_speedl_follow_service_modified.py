...
class HandSpeedLTrackingNode(Node):
    def __init__(self) -> None:
        ...
        # Hand Tracking -> VLA
        self.vla_signal_qos = QoSProfile(
        ...
        self.get_logger().info(
            "Tracking starts only after /arrived_goal Trigger service"
        )
        self.get_logger().warning(
            "Direct palm targeting enabled. Use conservative speed and robot safety limits."
        )
        self.get_logger().info(
            "Camera GUI code removed (forced headless mode)"
        )
    ...
    def _declare_parameters(self, script_dir: Path) -> None:
        defaults = {
            ...
            "max_approach_speed_mm_s": 400.0,
            "max_retreat_speed_mm_s": 300.0,
            "linear_acc_mm_s2": 1000.0,
    ...
    def _read_parameters(self) -> None:
        names = [
            ...
            "max_approach_speed_mm_s",
            "max_retreat_speed_mm_s",
            "linear_acc_mm_s2",
            "angular_acc_deg_s2",
            "command_time_sec",
    ...
    def _validate_parameters(self) -> None:
        ...
        if self.max_approach_speed_mm_s <= 0:
            raise ValueError("max_approach_speed_mm_s must be > 0")
        if self.max_retreat_speed_mm_s <= 0:
            raise ValueError("max_retreat_speed_mm_s must be > 0")
        if self.linear_acc_mm_s2 <= 0:
            raise ValueError("linear_acc_mm_s2 must be > 0")
    ...
    def _camera_to_base_mm(
        ...
        zyz_deg = self._rotation_matrix_to_zyz_deg(calibration_rotation)
        robot_pos = np.concatenate((calibration_position_mm, zyz_deg))
        ...
        try:
            ...
            self.status_text = f"Coordinate transform failed: {error}"
    ...
    def control_callback(self) -> None:
        ...
        # TCP에서 손바닥으로 향하는 단위벡터를 기준으로 접근/후퇴를 구분한다.
        # 직접 손바닥 좌표를 목표로 사용하므로 정상 추적 중에는 일반적으로
        # max_approach_speed_mm_s가 적용된다. 작업공간 제한 등으로 목표 방향이
        # 손의 반대쪽을 향할 때는 max_retreat_speed_mm_s가 적용된다.
        hand_direction = (palm_mm - tcp_mm) / hand_distance_mm
        moving_toward_hand = (
            float(np.dot(target_direction, hand_direction)) >= 0.0
        )
        speed_limit = (
            self.max_approach_speed_mm_s
            if moving_toward_hand
            else self.max_retreat_speed_mm_s
        )
        ...
        if now - self.last_log_time >= 1.0:
            self.get_logger().info(
                ...
                f"mode={'approach' if moving_toward_hand else 'retreat'}, "
                f"speed_limit={speed_limit:.1f} mm/s, "
